...
import saveChatTranscript from '@salesforce/apex/AgentforceChatController.saveChatTranscript';
import initializeAgentSession from '@salesforce/apex/AgentforceChatController.initializeAgentSession';
import getAgentRecommendation from '@salesforce/apex/AgentforceChatController.getAgentRecommendation';
import addMessageToConversation from '@salesforce/apex/AgentforceChatController.addMessageToConversation';
import endAgentSession from '@salesforce/apex/AgentforceChatController.endAgentSession';
import callElevenLabsTTS from '@salesforce/apex/ElevenLabsTTSController.callElevenLabsTTS';
...
export default class agentforceAssistant extends LightningElement {
    ...
    @api agentId = '0XxHu000000rXWLKA2'; // Place AgentforceID here from your URL
    @api headerText = 'Agentforce';
    @api elevenlabsApiKey = ''; // API key for ElevenLabs Voice
    ...
    @track voiceStatusText = 'Listening attentively...';
    ...
    connectedCallback() {
        ...
        try {
            const savedTheme = localStorage.getItem('agentforceAssistantDarkMode');
        ...
        if (this.isEmbedded) {
            // set default position and enable voice mode on Experience Cloud site
    ...
    initializeAgentforce() {
        ...
        const initializationTimeout = setTimeout(() => {
            ...
            this.updateInitializationMessage('Still working on connecting to Agent...');
        }, 5000);
        ...
        const performInitialization = () => {
            ...
            initializeAgentSession({ agentId: this.agentId })
                .then(result => {
                    ...
                    this.updateInitializationMessage('Connected to Agent successfully!');
    ...
    toggleDarkMode() {
        ...
        try {
            localStorage.setItem('agentforceAssistantDarkMode', this.isDarkMode);
        } catch (e) {
    ...
    toggleVoiceInput() {
        ...
        if (this.isVoiceMode) {
            ...
            this.voiceStatusText = 'Listening attentively...';
            
            // Directly start voice recognition without testing elevenlabsApi
    ...
    getAgentResponse(message) {
        ...
        const getResponse = () => {
            getAgentRecommendation({ sessionId: this.sessionId, message, elevenlabsApiKey: this.elevenlabsApiKey })
                .then(response => {
                    ...
                    if (response) {
                        ...
                        const errorMessage = "I am sorry, I do not have a response for your inquiry.";
                        if (this.isVoiceMode) {
    ...
    speakText(text) {
        ...
        const voiceStatusText = this.isFirstThinkingMessage ? "Agentforce AI initializing..." : "Preparing a response...";
...
this    .voiceStatusInterval = setInterval(() => {
        this.responseTimer++;
        const seconds = this.responseTimer;
        const voiceStatusText = this.isFirstThinkingMessage ? `Processing (${seconds}s)...` : `Creating voice response (${seconds}s)...`;
        this.voiceStatusText = voiceStatusText;
    }, 1000);

// Don't forget to clear this interval when the response is complete
// clearInterval(this.voiceStatusInterval);
        ...
        this.speakWithElevenLabsApi(cleanText)
            .then(() => {
                console.log('Speech completed using ElevenLabs');
            })
            .catch(error => {
                console.error('Error with ElevenLabs TTS, falling back to browser TTS:', error);
                
                // Check if ElevenLabs API returned a specific error
                if (error && error.message) {
                    console.warn('ElevenLabs error message:', error.message);
                }
    ...
    // Update the speakWithElevenLabsApi method with improved error handling and logging
    speakWithElevenLabsApi(cleanText) {
        return callElevenLabsTTS({ text: cleanText, elevenlabsApiKey: this.ElevenLabsApiKey })
            .then(result => {
            ...
            .catch(error => {
                console.error('Error calling ElevenLabs via proxy:', error);
    ...
    endChat() {
        ...
        this.messages = [...this.messages, {
            id: `msg_${Date.now()}`,
            text: 'Thanks for connecting with us today! We appreciate your time and look forward to helping you again soon.',
            sender: 'system',
            cssClass: 'message system-message',
            timestamp: this.getTimestamp()
        }];
    ...
    handleContinueSpeaking() {
        ...
        try {
            ...
            // 3. Stop any ElevenLabs audio specifically
